...
void main() {
  ...
  // Normalized screen coordinates, aspect-corrected from center
  vec2 uv = (gl_FragCoord.xy - 0.5 * iResolution.xy) / iResolution.y;
  uv.y *= -1.0;

  // Linear perspective camera ray with zoom control
  float fov = (3.14159 / 2.6) / max(uZoom, 0.2);
  vec3 dir = normalize(vec3(uv.x, uv.y, -1.0 / tan(fov * 0.5)));

  // Pitch camera toward horizon
...
/**
 * Raymarched WebGL wave field on a transparent canvas, so it can sit behind content.
 * Rendering pauses while scrolled out of view or the tab is hidden; callers gate it on
...
export function GradientWaves({
  ...
  // Set up the GL context once. Prop changes flow through the sync effect below.
  useEffect(() => {
